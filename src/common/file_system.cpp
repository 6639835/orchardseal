...
void* FileSystem::MapFile(const char* path, size_t offset, size_t size, size_t* mappedSize, bool readOnly)
{
    ...
    void* base = ::MapViewOfFile(mapping,
                                 ...
                                 static_cast<DWORD>(offsetValue >> 32U),
                                 static_cast<DWORD>(offsetValue & 0xffffffffU),
                                 size);
    ::CloseHandle(file);
    if (base == nullptr) {
        ::CloseHandle(mapping);
...
bool FileSystem::EnumFolder(const char* szFolder, bool bRecursive, enum_folder_callback filter, enum_folder_callback callback)
{
	string strFolder = szFolder;
