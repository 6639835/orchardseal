...
class jvalue::flat_map {
  ...
  private:
    ...
    int32_t _probe(const string& key) const {
        ...
        while (m_buckets[h] != EMPTY_SLOT) {
            if (m_entries[m_buckets[h]].first == key)
                return m_buckets[h];
            h = (h + 1) & mask;
        }
        return -1;
  ...
  public:
    ...
    flat_map(flat_map&& o) noexcept
        : m_entries(o.m_entries), m_buckets(o.m_buckets), m_size(o.m_size), m_cap(o.m_cap),
          m_bucket_count(o.m_bucket_count) {
        o.m_entries = NULL;
        o.m_buckets = NULL;
        o.m_size = 0;
        o.m_cap = 0;
...
class jpwriter {
  ...
  private:
    ...
    static int64_t _write_value_to_binary(const jvalue& pval, vector<bplist_object>& array_objects,
