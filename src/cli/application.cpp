...
int Application::RunAudit() const
{
    ...
    const std::string standardOutput = options_.auditFormat == AuditFormat::Json ? jsonReport : audit::ToText(report);
...
{
    return asset.Init(options_.certificateFile,
                      options_.privateKeyFile,
                      provisionFile,
                      options_.entitlementsFile,
                      options_.password,
                      options_.adhoc,
                      options_.sha256Only,
                      singleBinary);
...
{
    const bool shouldOnlyPrintInfo = !options_.adhoc &&
                                     options_.dylibFiles.empty() &&
    ...
    const bool signedOk = macho.Sign(&signAsset,
                                     options_.force,
                                     options_.bundleId,
                                     infoSha1,
                                     infoSha256,
...
int Application::ProcessBundleOrArchive()
{
    ...
    if (outputFile.empty()) {
        if (options_.install) {
            tempOutputFile = true;
            outputFile = FileSystem::GetRealPathV("%s/orchardseal_temp_%llu.ipa",
                                             options_.tempFolder.c_str(),
    ...
    if (inputIsZipFile) {
        ...
        tempFolder = true;
        enableCache = false;
        workingFolder = FileSystem::GetRealPathV("%s/orchardseal_folder_%llu",
                                            options_.tempFolder.c_str(),
                                            actionTimer.Reset());
        Logger::PrintV(">>> Unzip:\t%s (%s) -> %s ... \n",
                     options_.targetPath.c_str(),
                     FileSystem::GetFileSizeString(options_.targetPath.c_str()).c_str(),
                     workingFolder.c_str());
    ...
    if (options_.provisionFiles.size() > 1) {
        ...
        signedOk = bundle.SignFolder(&signAssets,
                                     workingFolder,
                                     options_.bundleId,
                                     options_.bundleVersion,
                                     options_.displayName,
                                     options_.dylibFiles,
                                     options_.removeDylibNames,
                                     force,
                                     options_.weakInject,
                                     enableCache,
                                     options_.removeProvision);
    } else {
        signedOk = bundle.SignFolder(&signAsset,
                                     workingFolder,
                                     options_.bundleId,
                                     options_.bundleVersion,
                                     options_.displayName,
    ...
    if (signedOk && !outputFile.empty()) {
        ...
        if (payloadPosition != std::string::npos && payloadPosition > 0) {
            ...
            if (!ZipArchive::Archive(baseFolder.c_str(), outputFile.c_str(), static_cast<std::uint32_t>(options_.zipLevel))) {
                Logger::Error(">>> Archive failed!\n");
                signedOk = false;
            } else {
                actionTimer.PrintResult(true,
                                        ">>> Archive OK! (%s)",
