...
{
    return StartsWith(name, "@executable_path/") ||
           StartsWith(name, "@loader_path/") ||
           StartsWith(name, "@rpath/") ||
           StartsWith(name, "/usr/lib/") ||
           StartsWith(name, "/System/Library/");
}
...
bool CommandLineOptions::ParseDylibInjectScope(const std::string& scope, std::uint32_t& parsedScope)
{
    ...
    for (std::string item : scopes) {
        ...
        } else if (item == "extensions" || item == "extension" || item == "plugins" ||
                   item == "plugin" || item == "appex") {
            result |= DYLIB_INJECT_EXTENSIONS;
        } else if (item == "frameworks" || item == "framework") {
            result |= DYLIB_INJECT_FRAMEWORKS;
        } else if (item == "files" || item == "file" || item == "binaries" ||
                   item == "binary" || item == "macho") {
...
ParseResult CommandLineOptions::Parse(int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc,
                              argv,
                              "dfvVa2LhiqwCRSEWUc:k:m:o:p:e:b:n:z:l:D:t:r:x:M:",
                              kOptions,
                              &optionIndex)) != -1) {
        switch (opt) {
        case 'd':
        ...
        case 'l': {
            ...
            result.options.dylibFiles.push_back(IsDylibLoadCommandName(value) ? value : FileSystem::GetFullPath(value.c_str()));
            break;
        }
        case OptDylibScope:
...
int CommandLineOptions::PrintUsage()
{
    ...
    Logger::Print("Usage: orchardseal [options] [-k privkey.pem] [-m dev.mobileprovision] [-o output.ipa] file|folder\n\n");
    ...
    Logger::Print("      --audit             Inspect an IPA, app bundle, folder, or Mach-O without modifying it.\n");
