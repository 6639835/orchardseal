...
bool CodeSignature::SlotParseCodeDirectory(uint8_t* pSlotBase, CS_BlobIndex* pbi)
{
	...
	if (Logger::IsDebug()) {
		if (1 == cdHeader.hashType) {
			...
			FileSystem::WriteFile("./.orchardseal_debug/CodeDirectory_SHA256.slot", (const char*)pSlotBase, uSlotLength);
		}
	}
...
bool CodeSignature::SlotBuildCodeDirectory(bool bAlternate,
	uint8_t* pCodeBase,
...
bool CodeSignature::SlotBuildCMSSignature(SigningAsset* pSignAsset,
...
bool CodeSignature::GetCodeSignatureExistsCodeSlotsData(uint8_t* pCSBase,
