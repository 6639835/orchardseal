...
{
	return ((uint32_t)(uint8_t)data[off] << 24) |
		   ((uint32_t)(uint8_t)data[off + 1] << 16) |
		   ((uint32_t)(uint8_t)data[off + 2] << 8) |
		   ((uint32_t)(uint8_t)data[off + 3]);
}
...
static bool DeflateData(const string& input, string& output)
{
	...
	int ret = compress2((Bytef*)&output[0], &outSize, (const Bytef*)input.data(), (uLong)input.size(), Z_BEST_COMPRESSION);
	if (Z_OK != ret) {
		output.clear();
		return false;
	}
...
static bool ConvertAppleOptimizedPng(const string& input, string& output)
{
	...
	if (bitDepth != 8 || colorType != 6 || compression != 0 || filter != 0 || interlace != 0) {
		Logger::WarnV(">>> Metadata: unsupported CgBI PNG format, copying original icon. bitDepth=%u colorType=%u interlace=%u\n", bitDepth, colorType, interlace);
		return false;
	}
