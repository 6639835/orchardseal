...
{
	if (cmsData == nullptr || cmsLength == 0) return nullptr;
...
static MachOSignInfo ExtractFromMachOData(std::uint8_t* base, std::uint32_t length)
{
    ...
    MachOSlice slice;
    if (!slice.Init(base, length) || slice.SignatureData() == nullptr ||
        slice.SignatureSize() < sizeof(CS_SuperBlob)) {
        return info;
    }
    ...
    if (LE(superBlob.magic) != CSMAGIC_EMBEDDED_SIGNATURE ||
        LE(superBlob.length) != signatureSize) {
        return info;
    }
    ...
    for (std::uint32_t index = 0; index < slotCount; ++index) {
        ...
        std::memcpy(&blobIndex,
                    signature + sizeof(CS_SuperBlob) + sizeof(CS_BlobIndex) * index,
                    sizeof(blobIndex));
...
static void PrintCertInfo(X509* cert, const string& fileTypeStr, bool showSigned, bool isSigned)
{
	...
	if (!org.empty()) Logger::PrintV(">>> Org:\t%s\n", org.c_str());
	if (!ou.empty()) Logger::PrintV(">>> Team:\t%s\n", ou.c_str());
...
int CheckSignedBinary(const string& strAppFolder)
{
	...
	string strInfoPlist;
	if (!FileSystem::ReadFile(strInfoPath.c_str(), strInfoPlist) || strInfoPlist.empty()) return -1;
