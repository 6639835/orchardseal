...
bool SigningAsset::GenerateCMS(void* pscert, void* pspkey, const string& strCDHashData, const string& strCDHashesPlist, const string& strCodeDirectorySlotSHA1, const string& strAltnateCodeDirectorySlot256, string& strCMSOutput)
{
	if (!pscert || !pspkey) {
		return CMSError();
	}
	...
	if (NULL != caCerts && StackContainsIssuerOf(caCerts, scert)) {
		...
		if (NULL == szIssuerCert) {
			Logger::ErrorV(">>> Unknown issuer hash 0x%08lx! No embedded WWDR intermediate matches and the p12 carries no usable CA chain.\n", issuerHash);
			return false;
		}
...
SigningAsset::SigningAsset(SigningAsset&& other) noexcept
	: SigningAsset()
{
...
bool SigningAsset::GenerateCMS(const string& strCDHashData, const string& strCDHashesPlist, const string& strCodeDirectorySlotSHA1, const string& strAltnateCodeDirectorySlot256, string& strCMSOutput)
{
	return GenerateCMS((X509*)certificate_, (EVP_PKEY*)privateKey_, strCDHashData, strCDHashesPlist, strCodeDirectorySlotSHA1, strAltnateCodeDirectorySlot256, strCMSOutput);
}
