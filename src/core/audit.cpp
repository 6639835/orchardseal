...
void ValidateIpaLayout(const fs::path& root, Report& report)
{
    ...
    if (!fs::is_directory(payload, error)) {
        AddIssue(report,
                 Severity::Error,
                 "IPA_PAYLOAD_MISSING",
                 "The archive does not contain a top-level Payload directory.",
                 "Payload");
        return;
    }
    ...
    if (error) {
        AddIssue(report,
                 Severity::Error,
                 "IPA_PAYLOAD_UNREADABLE",
                 "The Payload directory could not be enumerated completely.",
                 "Payload");
        return;
    }
    if (mainApps.size() != 1U) {
...
bool ParseProfile(const fs::path& path, bool embedded, ProfileRecord& record, std::string& error)
{
    ...
    if (record.info.applicationIdentifier.empty()) {
        record.info.applicationIdentifier = profile["Entitlements"]["com.apple.application-identifier"].as_cstr();
    }
...
int FindBestProfile(const std::vector<ProfileRecord>& profiles,
...
BundleRecord ParseBundle(const fs::path& root, const fs::path& path, const fs::path& mainBundle, Report& report)
{
    BundleRecord record;
    record.absolutePath = path;
    ...
    if (!info.read_plist_from_file("%s", infoPath.string().c_str())) {
        AddIssue(report, Severity::Error, "INFO_PLIST_INVALID", "Info.plist is missing or unreadable.", record.info.path);
        return record;
    }
    ...
    if (record.info.bundleId.empty()) {
        AddIssue(report, Severity::Error, "BUNDLE_ID_MISSING", "CFBundleIdentifier is missing.", record.info.path);
    }
    if (record.info.version.empty()) {
        AddIssue(report, Severity::Warning, "BUNDLE_VERSION_MISSING", "CFBundleVersion is missing.", record.info.path);
    }
    if (executableName.empty()) {
        AddIssue(report, Severity::Error, "BUNDLE_EXECUTABLE_MISSING", "CFBundleExecutable is missing.", record.info.path);
    } else {
        ...
        if (!fs::is_regular_file(record.executablePath, fsError)) {
            AddIssue(report,
                     Severity::Error,
                     "BUNDLE_EXECUTABLE_NOT_FOUND",
                     "The executable declared by CFBundleExecutable does not exist.",
                     record.info.executable);
        } else if (!IsMachOFile(record.executablePath)) {
            AddIssue(report,
                     Severity::Error,
                     "BUNDLE_EXECUTABLE_NOT_MACHO",
                     "The file declared by CFBundleExecutable is not a Mach-O binary.",
                     record.info.executable);
        }
    }
...
void ValidateBundleIdentifiers(const std::vector<BundleRecord>& bundles, Report& report)
{
    ...
    for (const BundleRecord& bundle : bundles) {
        ...
        const auto [iterator, inserted] =
            firstPathByIdentifier.emplace(bundle.info.bundleId, bundle.info.path);
        if (!inserted) {
            AddIssue(report,
                     Severity::Error,
                     "BUNDLE_ID_DUPLICATE",
...
bool IsMainExecutable(const fs::path& binary, const std::vector<BundleRecord>& bundles)
{
    return std::any_of(bundles.begin(), bundles.end(), [&](const BundleRecord& bundle) {
        return !bundle.executablePath.empty() && binary.lexically_normal() == bundle.executablePath.lexically_normal();
    });
}

bool AuditBinary(const fs::path& root,
                 const fs::path& path,
                 ...
                 Report& report)
{
    ...
    binary.signaturePresent = !binary.architectures.empty() &&
                              std::all_of(binary.architectures.begin(), binary.architectures.end(), [](const MachOSliceInfo& item) {
                                  return item.signaturePresent;
                              });
    const bool anySignature = std::any_of(binary.architectures.begin(), binary.architectures.end(), [](const MachOSliceInfo& item) {
        return item.signaturePresent;
    });
    binary.encrypted = std::any_of(binary.architectures.begin(), binary.architectures.end(), [](const MachOSliceInfo& item) {
    ...
    if (!binary.signaturePresent) {
        AddIssue(report,
                 Severity::Warning,
                 anySignature ? "BINARY_PARTIALLY_SIGNED" : "BINARY_UNSIGNED",
    ...
    if (checkArm64 && binary.mainExecutable) {
        const bool hasArm64 = std::any_of(binary.architectures.begin(), binary.architectures.end(), [](const MachOSliceInfo& item) {
            return item.architecture == "arm64" || item.architecture == "arm64e" || item.architecture == "arm64v8";
        });
        if (!hasArm64) {
            AddIssue(report,
                     Severity::Warning,
                     "MAIN_BINARY_ARM64_MISSING",
                     "The main executable does not contain an arm64-family architecture.",
                     displayPath);
        }
    }
...
void ScanMachOBinaries(const fs::path& root,
                       ...
                       Report& report)
{
    ...
    for (const fs::path& scanRoot : scanRoots) {
        std::error_code error;
        fs::recursive_directory_iterator iterator(scanRoot, fs::directory_options::skip_permission_denied, error);
        const fs::recursive_directory_iterator end;
        while (!error && iterator != end) {
            const fs::directory_entry& entry = *iterator;
            if (entry.is_symlink(error)) {
    ...
    for (const ProfileRecord& profile : profiles) {
        if (!profile.info.valid) {
            AddIssue(report,
                     Severity::Error,
                     "PROFILE_INVALID",
                     profile.parseError.empty() ? "Provisioning profile is malformed or lacks required identifiers."
                                                : profile.parseError,
                     profile.info.path);
            ...
            AddIssue(report,
                     Severity::Error,
                     "PROFILE_EXPIRED",
                     "Provisioning profile expired at " + profile.info.expiration + ".",
                     profile.info.path);
        }
    }
}

void MatchProfiles(const Request& request,
                   std::vector<ProfileRecord>& profiles,
                   const std::vector<int>& externalProfiles,
                   std::vector<BundleRecord>& bundles,
                   Report& report)
{
    for (BundleRecord& bundle : bundles) {
        ...
        if (candidates.empty()) {
            AddIssue(report,
                     ...
                     "No provisioning profile is available for compatibility checks.",
                     bundle.info.path);
            continue;
        }
        ...
        if (!profile.info.teamId.empty() && !bundle.info.bundleId.empty()) {
            ...
            if (pattern.empty()) {
                AddIssue(report,
                         Severity::Error,
                         "PROFILE_APPLICATION_ID_INVALID",
...
void ValidateSigningAssets(const Request& request,
                           const std::vector<ProfileRecord>& profiles,
                           const std::vector<int>& externalProfiles,
                           const std::vector<BundleRecord>& bundles,
                           Report& report)
{
    ...
    if (request.privateKeyFile.empty()) {
        AddIssue(report,
                 Severity::Error,
                 "PRIVATE_KEY_MISSING",
                 "A certificate was supplied without a private key or PKCS#12 file.",
                 request.certificateFile);
        return;
    }
    if (!FileSystem::IsFileExists(request.privateKeyFile.c_str())) {
        AddIssue(report, Severity::Error, "PRIVATE_KEY_NOT_FOUND", "Private key or PKCS#12 file was not found.", request.privateKeyFile);
        return;
    }
    if (!request.certificateFile.empty() && !FileSystem::IsFileExists(request.certificateFile.c_str())) {
        AddIssue(report, Severity::Error, "CERTIFICATE_NOT_FOUND", "Certificate file was not found.", request.certificateFile);
        return;
    }
    ...
    if (!request.adhoc && profilePaths.empty()) {
        AddIssue(report,
                 Severity::Error,
                 "SIGNING_PROFILE_REQUIRED",
                 "Signing assets cannot be validated without a provisioning profile.",
    ...
    for (const std::string& profilePath : profilePaths) {
        ...
        const int previousLogLevel = Logger::GetLogLevel();
        Logger::SetLogLevel(Logger::E_NONE);
        const bool valid = signingAsset.Init(request.certificateFile,
                                             request.privateKeyFile,
                                             profilePath,
                                             request.entitlementsFile,
                                             request.password,
                                             request.adhoc,
                                             request.sha256Only,
                                             bundles.empty());
        ...
        if (!valid) {
            ...
            AddIssue(report,
                     Severity::Error,
                     "SIGNING_ASSET_INVALID",
                     "The private key, certificate, password, and provisioning profile could not be validated as a compatible set.",
...
bool Service::Run(const Request& request, Report& report, std::string& error) const
{
    ...
    if (FileSystem::IsZipFile(input.string().c_str())) {
        ...
        temporaryDirectory = fs::path(request.tempDirectory) /
                             ("orchardseal_audit_" + std::to_string(Utility::GetMicroSecond()));
    ...
    if (report.inputType == "mach-o") {
        ...
        if (bundleRecords.empty()) {
            AddIssue(report,
                     ...
                     "No .app, .appex, .framework, or .xctest bundle was found; scanning the directory for Mach-O files.",
        ...
        if (report.binaries.empty()) {
            AddIssue(report, Severity::Error, "NO_SIGNABLE_BINARIES", "No Mach-O binaries were found.", DisplayPath(auditRoot, auditRoot));
        }
