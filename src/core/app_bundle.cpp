...
{
	return StartsWith(strDylib, "@executable_path/") ||
		   StartsWith(strDylib, "@loader_path/") ||
		   StartsWith(strDylib, "@rpath/") ||
		   StartsWith(strDylib, "/usr/lib/") ||
...
bool AppBundle::FindAppFolder(const string& strFolder, string& strAppFolder)
{
	...
	FileSystem::EnumFolder(strFolder.c_str(), true, [&](bool bFolder, const string& strPath) {
...
bool AppBundle::GetObjectsToSign(const string& strFolder, jvalue& jvInfo)
{
	...
	std::function<void(const string&)> findAllBundles = [&](const string& currentPath) {
		FileSystem::EnumFolder(currentPath.c_str(), false, NULL, [&](bool bFolder, const string& strPath) {
			if (bFolder) {
				if (FileSystem::IsPathSuffix(strPath, ".app") ||
					FileSystem::IsPathSuffix(strPath, ".appex") ||
...
bool AppBundle::InjectDylibsIntoTarget(MachOFile& macho, const string& strTargetFolder, bool bUseLoaderPath, vector<string>& arrCopiedDylibs)
{
...
bool AppBundle::SignNode(jvalue& jvNode)
{
	...
	Logger::PrintV(">>> SignFolder: %s, (%s)\n", ("/" == strFolder) ? Utility::GetBaseName(appFolder_.c_str()) : strFolder.c_str(), strBundleExe.c_str());
	...
	if (ShouldInjectDylibsIntoNode(jvNode)) {
		...
		if (!injectionDylibs_.empty()) {
			if (!InjectDylibsIntoTarget(macho, strBaseFolder, ShouldUseLoaderPathForBundleType(strBundleType), arrCopiedDylibs)) {
				return false;
			}
	...
	if (signingAssets_ && !removeProvision_ && "framework" != strBundleType) {
		...
		for (auto it = signingAssets_->rbegin(); it != signingAssets_->rend(); ++it) {
			if (endsWith(it->ApplicationIdentifier(), strBundleId)) {
				signingAsset_ = &(*it);
				if (!FileSystem::WriteFileV(signingAsset_->ProvisioningData(), "%s/%s/embedded.mobileprovision", appFolder_.c_str(), strFolder.c_str())) {
		...
		if (jvInfo.has("NSExtension")) {
			if (jvInfo["NSExtension"].has("NSExtensionAttributes")) {
				if (jvInfo["NSExtension"]["NSExtensionAttributes"].has("WKAppBundleIdentifier")) {
					...
					Logger::PrintV(">>> BundleId: \t%s -> %s, NSExtension-NSExtensionAttributes-WKAppBundleIdentifier\n", strOldWKBundleID.c_str(), strNewWKBundleID.c_str());
				}
			}
		}
...
bool AppBundle::ModifyBundleInfo(const string& strBundleId, const string& strBundleVersion, const string& strDisplayName)
{
	jvalue jvInfo;
...
bool AppBundle::SignFolder(SigningAsset* pSignAsset,
							...
							bool bRemoveProvision)
{
	...
	if (!pSignAsset->ProvisioningData().empty()) {
		if (!FileSystem::WriteFileV(pSignAsset->ProvisioningData(), "%s/embedded.mobileprovision", appFolder_.c_str())) { // embedded.mobileprovision
			Logger::ErrorV(">>> Can't write embedded.mobileprovision!\n");
			return false;
		}
	}
...
bool AppBundle::SignFolder(list<SigningAsset>* pSignAssets,
						const string& strFolder,
						const string& strBundleId,
						const string& strBundleVersion,
						const string& strDisplayName,
						...
						bool bRemoveProvision)
{
	...
	return SignFolder(&signingAssets_->front(), strFolder, strBundleId, strBundleVersion, strDisplayName, arrInjectDylibs, arrRemoveDylibNames, bForce, bWeakInject, bEnableCache, bRemoveProvision);
}
