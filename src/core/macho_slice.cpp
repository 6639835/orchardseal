...
bool MachOSlice::Init(std::uint8_t* base, std::uint32_t length) {
    ...
    if (!RangeFits(headerSize_, commandBytes, length_) || commandCount > commandBytes / sizeof(load_command)) {
        return false;
    }
    ...
    for (std::uint32_t index = 0; index < commandCount; ++index) {
                ...
                for (std::uint32_t sectionIndex = 0; sectionIndex < sectionCount; ++sectionIndex) {
                    ...
                    if (FixedNameEquals(current->sectname, "__text")) {
                        ...
                        if (offset > commandsEndOffset) {
                            ...
                            loadCommandsFreeSpace_ =
                                loadCommandsFreeSpace_ == 0 ? available : std::min(loadCommandsFreeSpace_, available);
                        }
            ...
            if (FixedNameEquals(segment->segname, "__TEXT")) {
                ...
                for (std::uint32_t sectionIndex = 0; sectionIndex < sectionCount; ++sectionIndex) {
                    ...
                    if (FixedNameEquals(current->sectname, "__text")) {
                        ...
                        if (!RangeFits(offset, size, length_) || size > std::numeric_limits<std::size_t>::max()) {
                            return false;
                        }
        ...
        case LC_LOAD_WEAK_DYLIB: {
            ...
            if (nameOffset < sizeof(dylib_command) || !ContainsTerminatedString(command, commandSize, nameOffset)) {
                return false;
            }
            break;
        }
...
void MachOSlice::PrintInfo() {
    ...
    Logger::PrintV("\tSpareLength: \t%d (%s)\n", length_ - codeLength_ - signatureLength_,
                   Utility::FormatSize(length_ - codeLength_ - signatureLength_).c_str());
...
bool MachOSlice::BuildCodeSignature(SigningAsset* pSignAsset, bool bForce, const string& strBundleId,
    ...
    if (!pSignAsset->Sha256Only()) {
        if (!CodeSignature::SlotBuildCodeDirectory(false, base_, codeLength_, pCodeSlots1Data, uCodeSlots1DataLength,
                                                   execSegmentLimit_, uExecSegFlags, strBundleId, pSignAsset->TeamId(),
                                                   strInfoSHA1, strRequirementsSlotSHA1, strCodeResourcesSHA1,
                                                   strEntitlementsSlotSHA1, strDerEntitlementsSlotSHA1, IsExecute(),
                                                   pSignAsset->IsAdHoc(), strCodeDirectorySlot)) {
            Logger::Error(">>> Build SHA1 CodeDirectory failed!\n");
            return false;
        }
    }

    if (!CodeSignature::SlotBuildCodeDirectory(true, base_, codeLength_, pCodeSlots256Data, uCodeSlots256DataLength,
                                               execSegmentLimit_, uExecSegFlags, strBundleId, pSignAsset->TeamId(),
                                               strInfoSHA256, strRequirementsSlotSHA256, strCodeResourcesSHA256,
                                               strEntitlementsSlotSHA256, strDerEntitlementsSlotSHA256, IsExecute(),
                                               pSignAsset->IsAdHoc(), strAltnateCodeDirectorySlot)) {
    ...
    if (!pSignAsset->IsAdHoc()) { // adhoc remove cms signature slot
        if (!CodeSignature::SlotBuildCMSSignature(pSignAsset, strCodeDirectorySlot, strAltnateCodeDirectorySlot,
                                                  strCMSSignatureSlot)) {
...
bool MachOSlice::Sign(SigningAsset* pSignAsset, bool bForce, const string& strBundleId, const string& strInfoSHA1,
                      const string& strInfoSHA256, const string& strCodeResourcesData) {
    ...
    if (!BuildCodeSignature(pSignAsset, bForce, strBundleId, strInfoSHA1, strInfoSHA256, strCodeResourcesSHA1,
